...
class BalanceService:
    ...
    def __init__(self, account_repo: AccountRepo):
        ...
        # {account_id: {currency_id: quants}} -- account own balances
    ...
    def invalidate(self, account_id: int, currency_id: int | None = None) -> None:
        ...
        while current_id is not None:
            self._cache.pop(current_id, None)
    ...
    def get_balance(self, account_id: int, include_children: bool = True) -> dict[int, int]:
        ...
        if not include_children:
            return self._get_own_balance(account_id)

        result: dict[int, int] = defaultdict(int)
        for cid, quants in self._get_own_balance(account_id).items():
            result[cid] += quants
    ...
    def _get_own_balance(self, account_id: int) -> dict[int, int]:
        if account_id in self._cache:
            return self._cache[account_id]
        balance = self.account_repo.get_balance(account_id)
        self._cache[account_id] = balance
        return balance
